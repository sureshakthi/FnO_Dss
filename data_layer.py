...
# Auto-detect proxy: use office proxy if reachable, else connect directly
...
def _test_proxy() -> bool:
    """Return True if the corporate proxy is reachable (office/VPN)."""
    try:
...
def _fetch_chart(ticker: str, interval: str, range_str: str) -> pd.DataFrame:
    """Low-level: call Yahoo Finance chart API and return OHLCV DataFrame."""
...
def fetch_daily_data(symbol_key: str, days: int = 60) -> pd.DataFrame:
    """Fetch daily OHLCV data (up to 100 days)."""
...
def fetch_vix() -> dict:
    """
    Fetch India VIX (^INDIAVIX) — measures market fear/volatility.
    Returns dict with 'value', 'prev', 'change_pct', 'level'.
    VIX < 13 = complacent, 13–20 = normal, 20–30 = caution, >30 = panic.
    """
